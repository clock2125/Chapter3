...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator transAnimator = ObjectAnimator.ofFloat(imageView, "alpha", 1.0f,0.5f);
        transAnimator.setRepeatCount(ValueAnimator.INFINITE);
        transAnimator.setInterpolator(new LinearInterpolator());
        transAnimator.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        transAnimator.setRepeatMode(ValueAnimator.REVERSE);
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,scaleXAnimator,scaleYAnimator,transAnimator);
        animatorSet.start();
    }
}
